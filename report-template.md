...
```text
（在这里填写）
测试准确率 34.60% 是随机猜测基线（10%）的约 3.5 倍，说明预训练后的 encoder 确实学到了有意义的图像表征。
虽然 34.60% 的绝对准确率不高，但考虑到以下限制条件，这个结果是合理的：
1. 仅使用了 5000 张训练图像（CIFAR-10 全集的 10%）进行自监督预训练；
2. 仅训练了 10 个 epoch，对比学习通常需要更多 epoch 才能充分收敛；
3. 使用了小型 CNN encoder（约 270 万参数），而非 ResNet 等更强 backbone；
4. batch size 为 128，负样本数量有限（SimCLR 论文使用 4096+ 的 batch size）。
Linear Probe 阶段的训练准确率从 28.78% 稳步提升至 33.72%，
说明线性分类器在冻结的特征上能够有效学习，验证了 encoder 表征的质量。
```
...
（在这里填写）
遇到的问题及解决：
1. GPU 显存限制：RTX 4060 的 8GB 显存限制了 batch size 的上限。
2. 数值稳定性：NT-Xent loss 计算中需要排除自身相似度（对角线），初始使用 -inf 填充被掩码的位置，配合 CrossEntropyLoss 自动忽略 -inf 位置，保证了数值稳定性。
3. 双视图增强一致性：确保同一张图像的两个增强视图通过同一个 transform pipeline 独立生成，产生不同的增强结果（平均绝对差异约 0.08），满足了对比学习的基本要求。
改进方向：
1. 增加预训练 epoch：从 10 增加到 50-100，让对比学习更充分收敛；
2. 增大 batch size：batch size 越大，负样本越丰富，对比学习效果越好；
3. 使用更强的 encoder：将小型 CNN 替换为 ResNet-18（适配 CIFAR-10），增强表征能力；
4. 增加训练样本：使用全部 50000 张训练图像进行预训练；
5. 调优 temperature：尝试不同的 temperature 值（如 0.1、0.07、1.0）寻找最优设置；
6. 增加数据增强：加入 GaussianBlur，SimCLR 论文表明模糊增强对性能有提升；
7. 更深的 projection head：尝试 3 层 MLP（如 512→512→512→128）；
8. 使用更强的学习率调度策略：如 warmup + cosine decay。
```

## 12. AI 对话过程记录

- 录制工具：claude code for vscode, trae work for vscode
- 对话链接：无
- 使用的 AI 模型：deepseekV4pro
- 累计对话时长 / 会话数：xx
...
- 仓库地址：https://github.com/SONGVXI/mini-simclr.git
- 总 commit 数：
...
88eccb5 (HEAD -> main, origin/main) first commit
```
